                ...
                <nav className="shadow-lg shadow-gray-20 w-full fixed top-0 z-10">
                    <div className="max-sm:flex sm:flex max-sm:justify-between sm:justify-between">
                        ...
                        <div className='w-[100%] flex justify-between'>
                            {/* logo */}
                            <div className='md:order-1 w-1/2 max-sm:w-full sm:w-full flex items-center justify-center py-2 bg-navbg1 max-sm:bg-white'>
                                <div className="flex items-center justify-start">
                                    <img src={logo} alt="Logo" className="lg:h-[74px] lg:w-[80px] max-sm:h-[63px] sm:h-[50px] sm:w-[60px] bg-white rounded-full p-[2px] max-sm:w-[60px]" />
                                    <span className="text-white text-lg max-sm:text-base max-md:text-base bg-orange-500 px-[6px] py-[2px] rounded-sm">Pushing up performance</span>
                                </div>
                            </div>
                            ...
                            <div className='md:order-2 flex items-center justify-end logobg  w-1/2 sm:w-full hidden md:block px-5'>
                                <div className="flex items-center justify-center h-full">
                                    <div className="flex flex-col items-center">
                                        <ul className="navlinks flex flex-col items-center  md:flex-row md:gap-10 max-sm:gap-4 sm:gap-4">
                                            {menuItems.map((menuItem, index) => (
                                                <li key={index}>
                                                    {menuItem.dropdown ? (
                                                            ...
                                                            {activeItem === index && (
                                                                <ul className="absolute top-full w-[240px] left-0 bg-gray-300 shadow-lg rounded-md py-2 px-4">
                                                                    {menuItem.dropdown.map((item, i) => (
                                                                        <li key={i} className='text-gray-900 hover:text-orange-500 text-lg'>
                                                                            <NavLink to={item.link} activeClassName="active" onClick={handleItemClick}>
                                                                                {item.label}
                                                                            </NavLink>
                                                                        </li>
                                                                    ))}
                                                        ...
                                                        <NavLink to={menuItem.link} activeClassName="active" onClick={handleItemClick}>
            ...
            {showSidebar && (
                <div className="fixed inset-0 bg-navbg1 text-white z-50 w-[310px] py-3 px-4">
                    ...
                    <div className=' flex justify-end '>
                      <button onClick={closeSidebar} className="text-gray-900 bg-white ">
                   ...
                   <img src={logo} alt="Logo" className="lg:h-[74px] lg:w-[80px] max-sm:h-[63px] sm:h-[50px] sm:w-[60px] bg-white rounded-full p-[2px] max-sm:w-[60px]" />
                    ...
                    <div>
                        <ul className="navlinks flex flex-col  p-2  md:flex-row md:gap-10 max-sm:gap-4 sm:gap-4">
                            {menuItems.map((menuItem, index) => (
                                <li key={index}>
                                    {menuItem.dropdown ? (
                                        <div
                                            ...
                                            className=" relative "
                                        >
                                            <p className="text-white text-lg cursor-pointer">{menuItem.label}</p>
                                            {activeItem === index && (
                                                <ul className="absolute top-full w-auto left-0 bg-white shadow-lg rounded-md py-2 px-4 ">
                                                    {menuItem.dropdown.map((item, i) => (
                                                        <li key={i} className='text-gray-600 hover:text-orange-500 text-lg'>
                                                            <NavLink to={item.link} activeClassName="active" onClick={handleItemClick}>
                                                                {item.label}
                                                            </NavLink>
                                                        </li>
                                        ...
                                        <NavLink to={menuItem.link} activeClassName="active" onClick={handleItemClick}>
                                            <p className={`text-white text-xl hover:text-orange-500 ${activeItem === index ? 'text-orange-500 ' : ''}`}>{menuItem.label}</p>
                                        </NavLink>
                                    )}
                                </li>
                            ))}
